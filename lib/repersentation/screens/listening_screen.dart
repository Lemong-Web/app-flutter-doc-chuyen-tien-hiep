    ...
    class _ListeningScreenState extends State<ListeningScreen> {
      ...
      Future<void> _speakLongText(String text, {int startPosition = 0}) async {
        ...
        String textToSpeak = text.substring(startPosition);
        _textQueue = textToSpeak
            .split(RegExp(r'(?<=[.!?…]) '))
      ...
      currentStartPosition = currentCharPosition;
    ...
    void _resumeSpeaking() async {
      if (!isPause) return;
        ...
        String remaingText = chapterTexts[currentIndex].substring(currentCharPosition);

        _textQueue = remaingText.split(RegExp(r'(?<=[.!?...]) ')).where((s) => s.trim().isNotEmpty).toList();

        if(_textQueue.isNotEmpty) {
  //     String firstSentence = _textQueue.removeAt(0);
  //     currentCharPosition = currentStartPosition + firstSentence.length + 1;
          _speakNext();
        } else {
          setState(() {
            _isReading = false;
    ...
    void _resetPage() {
      ...
      void _setSpeachRate() async {
        ...
        if (_isReading && !isPause && _textQueue.isNotEmpty) {
          await flutterTts.stop();
      ...
      void initState() {
          ...
          flutterTts.setProgressHandler((text, start, end, word) {
            if(mounted) {
              setState(() {
                currentPosition = currentStartPosition + start;
                currentCharPosition = currentStartPosition + start;   
              });
            }
          });
                              ...
                              IconButton(
                                onPressed: () {
                                    ...
                                    _resumeSpeaking();
                                      ...
                                      _speakLongText(chapterTexts[currentIndex], startPosition: 0);
                                      setState(() {
                                        _isReading = true;
                                      });
                                    }
